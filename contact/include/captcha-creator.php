...
class FGCaptchaCreator
{
    ...
    function __construct($captcha_var_name)
    {
    ...
    function Validate()
    {
        return isset($_POST[$this->captcha_var_name]) && $this->ValidateCaptcha($_POST[$this->captcha_var_name]);
    }
    ...
    {
        return 'captcha_code'; // Static name
    }
    ...
    function ValidateCaptcha($userscode)
    {
        ...
        $session_var = $this->getSessionVarName();
        return isset($_SESSION[$session_var]) && $_SESSION[$session_var] === $enc_user_answer;
    }
    ...
    function DrawLines()
    {
        ...
        $linecolor = imagecolorallocate($this->image, $this->linecolor, $this->linecolor, $this->linecolor);
    ...
    function DrawChars($code)
    {
        ...
        for ($i = 0; $i < $N; $i++) {
            ...
            $x = $left < 0 ? abs($left) : 0;
            imagettftext($image_char, $font_size, $angle, $left, $top, $textcolor, $this->font_file, mb_substr($code, $i, 1, 'UTF-8'));
            $dispwidth = $spacing - 3;
            $y = ($this->margin_y / 2);
            $dispheight = $this->height - $y;
            imagecopyresampled($this->image, $image_char, $i * $spacing, $y, 0, 0, $dispwidth, $dispheight, $charwidth, $charheight);
        }
    }
    ...
    function Distort($im)
    {
        ...
        for ($x = 0; $x < $width; $x++) {
            ...
            imagecopyresampled($image_out, $im, $x, $d, $x, 0, 1, $height - $d, 1, $height);
        }
        return $image_out;
    }
